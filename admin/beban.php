...
<h3 class="animated zoomInRight"><span class="glyphicon glyphicon-briefcase"></span>  Beban Administrasi Santri</h3>
<h3 class="animated zoomInDown"><button style="margin-bottom:20px" data-toggle="modal" data-target="#myModal" class="btn btn-warning col-md-2"><span class="glyphicon glyphicon-plus"></span>Add Data</button></h3>
<a class="btn" href="pay.php"><span class="glyphicon glyphicon-arrow-left"></span>  Back To Payment</a>
<a class="btn" href="exc.php"><span class="glyphicon glyphicon-arrow-right"></span>  Lihat data Exc</a>
...
<form action="" method="get">
	<div class="input-group col-md-5 col-md-offset-7">
		...
		<select type="submit" name="thn" class="form-control" onchange="this.form.submit()">
			<option>.. Tahun ..</option>
			<?php 
			$pil=mysql_query("select distinct thn from beban order by thn desc");
...
<?php 
if(isset($_GET['thn'])){
	echo "<h4> Data Beban Santri Tahun  <a style='color:blue'> ". $_GET['thn']."</a></h4>";
}
?>
<table class="table table-hover">
	...
	if(isset($_GET['thn'])){
		...
		$brg=mysql_query("select * from beban where thn = '$tah' order by thn desc");
	}else{
		$brg=mysql_query("select * from beban order by thn desc");
	}
	$no=1;
	while($b=mysql_fetch_array($brg)){
		...
		?>
		<tr>
			<td><a href="beban_edit.php?no=<?php echo $b['no']; ?>"><?php echo $b['no'] ?></a></td>
	...
	<div class="modal-dialog">
		<div class="modal-content">
			...
			<div class="modal-body">
				<form action="beban_act.php" method="post">
