...
$no=1;
$query=mysql_query("select * from finance where period = $per order by tgl asc");
while($lihat=mysql_fetch_array($query)){
	$fak=$lihat['no'];
	$pdf->SetFont('Arial','',8);
	$fak=sprintf('%04d' , $fak);
	$pdf->Cell(1, 0.8, $fak,1,0, 'C');
	$pdf->Cell(2.5, 0.8, $lihat['tgl'],1, 0, 'C');
	$pdf->Cell(1.5, 0.8, $lihat['akun'],1, 0, 'C');
	$pdf->Cell(3, 0.8, substr($lihat['vendor'],0,19), 1, 0,'L');
	$pdf->Cell(11.5, 0.8, $lihat['uraian'], 1, 0,'L');
	$pdf->Cell(3, 0.8, $lihat['ket'], 1, 0,'C');
	$pdf->Cell(2.5, 0.8, number_format($lihat['debit'],0,'',','), 1, 0,'R');
	$pdf->Cell(2.5, 0.8, number_format($lihat['kredit'],0,'',','), 1, 1,'R');
	$no++;
}

$pdf->SetFont('Arial','B',10);
$jml=mysql_query("select sum(debit) as jd, sum(kredit) as jk from finance where period = $per");
while($see=mysql_fetch_array($jml)){
	$jum=$see['jk']-$see['jd'];
	$pdf->Cell(22.5, 0.8, '', 0, 0,'C');
	$pdf->Cell(5, 0.8, number_format($jum,0,'',','), 0, 1,'R');
}
